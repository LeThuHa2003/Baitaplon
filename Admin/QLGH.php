<?php 
    ...
    if (isset($_SESSION['Admin'])) { 
        ...
        $str = "SELECT * FROM  GioHang";
        $result = mysqli_query($conn, $str);

        //xóa 1 thành viên
        if (isset($_GET['del_idgh']))
        {
            $magh = $_GET['del_idgh'];
            $queryDelete = "DELETE FROM GioHang WHERE MaGioHang = '$magh'";
            mysqli_query($conn, $queryDelete) or die('query failed');
            mysqli_close($conn);
            header("Location: QLGH.php?deleted_magh=$magh");
        }
?>
...
<html>
    ...
    <body >
            ...
            <div class="container" style="overflow-x: auto;">
                <!-- <form action=""> -->
                    <table class="tbl_qlsp">
                        ...
                        {
                            $search = $_GET['search'];
                            $str1 = "SELECT * FROM GioHang WHERE MaGioHang like '%$search%' OR id like '%$search%' OR MaSP like '%$search%' OR TenSP like '%$search%'"; 
                            ...
                            { 
                                ...
                                ?>
                            <tr class="items">
                                <td style="text-align: center;"> <?php echo $row['MaGioHang']?></td>
                                ...
                                <td style="text-align: center;"> <a href="QLGH.php?del_idgh=<?php echo $row['MaGioHang']?>" onclick="return confirm('Bạn có chắc chắn muốn xóa sản phẩm này không?');" class="fas fa-trash"></a></td>
                            </tr>
                                <?php }} else {
        ...
        <script>
            var deleted_magh = "<?php echo isset($_GET['deleted_magh']) ? $_GET['deleted_magh'] : ''; ?>";
            if (deleted_magh !== '') {
                alert("Bạn đã xóa sản phẩm có mã giỏ hàng " + deleted_magh);
            }
        </script>
